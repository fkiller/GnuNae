...
4. `.github/workflows/release.yml` starts on the tag. For owner-approved
   ...
   tag, manually dispatch `release.yml` with `release_mode=mas-only` for MAS
   only, `release_mode=stores-only` for both MAS and Microsoft Store jobs, or
   `release_mode=msstore-only` for a Microsoft Store-only resubmission after a
   certification issue.
